...
class SPNS_Agent: public PatrolAgent {
...
void SPNS_Agent::init(int argc, char** argv) {
...
int SPNS_Agent::compute_next_vertex() {
    return spatial_priority_network(current_vertex, vertex_web, instantaneous_idleness, tab_intention, NUMBER_OF_ROBOTS, dimension, node_node_distances, last_node, last_last_node, adjacency_matrix);
}


void SPNS_Agent::send_results() {   
    int value = ID_ROBOT;
    if (value==-1){value=0;}
...
void SPNS_Agent::receive_results() {
